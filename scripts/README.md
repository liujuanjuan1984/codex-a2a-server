...
- [`scripts/deploy_light.sh`](./deploy_light.sh):
  ...
  reasoning combinations before launch. `start` detaches through `nohup` +
  `setsid`, records the real service PID under `run/light/`, and keeps each
  launch on its own timestamped log file under `logs/light/`, while
  `logs/light/<instance>.log` stays as the stable alias to the latest launch.
